...
use rustgo_protocol::{
    ...
    ErrorMessage, FrameCodec, FrameError, HEADER_LEN, Heartbeat, MAGIC, Message, MessageId,
    OpenTcpStream, ProtocolErrorCode, ProtocolVersion, RegisterTunnels, ServerChallenge,
...
fn every_message_family_round_trips() {
    ...
    for message in messages() {
        let encoded = codec.encode(VERSION, 0x1020, &message).unwrap();
        let decoded = codec.decode_exact(&encoded).unwrap();
        assert_eq!(decoded.version, VERSION);
        assert_eq!(decoded.flags, 0x1020);
...
fn header_uses_fixed_magic_and_network_byte_order() {
    ...
    assert_eq!(&encoded[10..12], &[0x10, 0x20]);
    assert_eq!(
