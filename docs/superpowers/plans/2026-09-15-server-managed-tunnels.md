...
- [x] Add failing codec roundtrip, old-version state rejection, direction, bounds and invalid snapshot tests; run `cargo test -p rustgo-protocol -p rustgo-config`.
```rust
assert!(!ProtocolVersion::V0_3.supports_managed_configuration());
assert!(ProtocolVersion::V0_4.supports_managed_configuration());
```
- [x] Implement serde data contracts and validation (existing name/address/port/ACL rules); snapshot apply replaces only three collections and requires locally enabled P2P when needed.
- [x] Encode/decode new message IDs and allow request/snapshot only during authenticated pre-registration, report only client-to-server active.
- [x] Re-run focused tests and commit task files.
...
- [x] Write regression proving an empty saved snapshot survives database reopen and a later nonempty sync; run its test to observe failure.
```rust
...
```
- [x] Implement bounded durable transactional import/update and stale-report rejection. Keep identity separate from display name; a different identity cannot inherit an old name's snapshot.
- [x] Test conflicts, stale reports, rename, same-name replacement and invalid input; commit only owned files.
...
- [x] Add tests for authenticated snapshot exchange and unauthenticated/CSRF write rejection.
- [x] Open dedicated store from optional `[managed_tunnels] database_path` configuration independent of enrollment. Resolve dynamic stable internal ID or static public-key fingerprint from authenticated identity; update name mapping only after verified auth.
- [x] Exchange snapshot before registration; reject oversized/invalid JSON and registration not matching the desired snapshot. Persist per-item report only for the bound identity and revision.
- [x] Implement GET `/api/v1/clients/{name}/tunnels` and POST same path using `{expected_revision, action:"add"|"delete", kind:"tunnel"|"export"|"forward", item|name}`. Validate candidate snapshot transactionally, then terminate only the affected client's generation.
- [x] Add online and supported indicators; storage errors return unavailable, conflicts return 409. Ensure disabled/deleted identities cannot expose stale snapshots through name reuse.
- [x] Run server tests and commit server integration.
...
- [x] Add a real handshake test proving a server empty snapshot overrides nonempty local tunnels before registration.
- [x] Send initial snapshot post-auth, validate received snapshot against local P2P policy and apply before tunnel registration. Make effective config accessible to generation startup.
- [x] Recreate export registry and P2P runtime for each generation and await shutdown before reuse; report actual per-item readiness/failure once generation setup completes. CLI and GUI use this same code.
- [x] Expose managed ownership and effective collections through ClientStatus; GUI refreshes rows and disables local writes to managed collections.
- [x] Test reconnect removal and runtime cleanup, run client/GUI tests, commit owned changes.
...
- [x] Test three-kind rendering, delete names, pending/failed display and revision request payloads.
- [x] Add detail table, type-specific forms, ACL explanation, deletion action and interruption notice. Fetch managed detail separately from existing names-only inventory.
- [x] Exercise TCP/UDP forwarding and export/forward lifecycle, offline add/delete, process restart, port conflicts and old-client downgrade using real processes.
- [x] Run fmt, clippy, relevant workspace tests and release builds; obtain code review and address findings.
- [x] Deploy with existing backup/rollback workflow, update local GUI and verify live all-kind inventory and a reversible test tunnel lifecycle. Record exact verification and remaining limitations.
